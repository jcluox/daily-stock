...
def compute_top_n(window_days: int, history: pd.DataFrame) -> pd.DataFrame:
    """Rank stocks by cumulative trading value over the last `window_days` trading days.
    ...
    stocks with columns [code, name, total_trading_value, days_available,
    insufficient_history, rank].
    """
    if history.empty:
        return pd.DataFrame(
            columns=["code", "name", "total_trading_value", "days_available", "insufficient_history", "rank"]
        )
    ...
    grouped = grouped.sort_values("total_trading_value", ascending=False).reset_index()
